...
class ClienteAPITestCase(APITestCase):
    ...
    def test_get_clientes_list(self):
        """Testa o endpoint GET /clientes/"""
        response = self.client.get(self.url_list)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertGreaterEqual(len(response.data), 1)
        print(f"{Cores.VERDE}Teste para endpoint GET /clientes/ concluído com sucesso.{Cores.RESET}")

    def test_get_cliente_detail(self):
        """Testa o endpoint GET /cliente/detail/<int:pk>"""
        response = self.client.get(self.url_detail)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['NomePessoa'], "João Silva")
        print(f"{Cores.VERDE}Teste para endpoint GET /cliente/detail/<int:pk> concluído com sucesso.{Cores.RESET}")

    def test_post_cliente(self):
        """Testa o endpoint POST /cliente/create/"""
        data = {
        ...
        print(f"{Cores.VERDE}Teste para endpoint POST /cliente/create/ concluído com sucesso.{Cores.RESET}")

    def test_update_cliente(self):
        """Testa o endpoint PUT /cliente/update/<int:pk>"""
        data = {
        ...
        print(f"{Cores.VERDE}Teste para endpoint PUT /cliente/update/<int:pk> concluído com sucesso.{Cores.RESET}")

    def test_delete_cliente(self):
        """Testa o endpoint DELETE /cliente/delete/<int:pk>"""
        response = self.client.delete(self.url_delete)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(Cliente.objects.count(), 0)
        print(F"{Cores.VERDE}Teste para endpoint DELETE /cliente/delete/<int:pk> concluído com sucesso.{Cores.RESET}")